...
# Conclusion 
This study set out to create a county level data set of demographic, health, and COVID-19 data set to then create a predictive model to guess the rate of death from COVID-19 for those who've contracted the disease (deaths/cases). The modelling  approaches included a simple "Just the Average" approach, a linear regression model, a regression tree model with bootstrap aggregating, and finally a random forest approach. The best performing model was the random forest which had an RMSE of less than .005%. 

While the random forest model preforms relatively well, it doesn't necessarily provide information to guide policy. Understanding which counties are experiencing higher death rates per case from a contagious disease could help policy makers decide where resources such as personal protective gear, ventilators, medical workers, etc. should be allocated. In addition, it may help us understand the impacts of long term environmental conditions such as years of poor air quality.

While the random forest approach lets us see the importance of different variables, it doesn't help us intuitively understand these variables or how they interact with other observations in our data set. More granular data, such as Census tract level information, could help address these challenges.
